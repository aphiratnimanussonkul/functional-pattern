...
final class WordCount(val filePath: String):
  ...
  def run(): Unit =
    ...
    val normalized = rawText.toLowerCase //Normalize Text
    ...
    val wordList = text.split("\\s+").filter(_.nonEmpty).toList // Word list

    val totalWords = wordList.size // Total words
    val uniqueCount = wordList.distinct.size // Unique count

    val countMap = scala.collection.mutable.Map.empty[String, Int]
    var idx = 0
    while idx < wordList.size do
      val w = wordList(idx)
      countMap(w) = countMap.getOrElse(w, 0) + 1 // Count words
      idx += 1
    val top5 = countMap.toList.sortBy(-_._2).take(5) // Top 5 words

    val report = new StringBuilder // Report
